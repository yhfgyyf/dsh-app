...
function harness(options: { review?: (spec: string, signal: AbortSignal) => Promise<any>; prepare?: (signal: AbortSignal) => Promise<any>; install?: (spec: string, options: any) => Promise<any>; inspect?: () => any } = {}) {
  let now = Date.now();
  ...
  const registrations: any[] = [], disposers: (() => void)[] = [], reviews: { spec: string; signal: AbortSignal }[] = [], prepares: AbortSignal[] = [], installs: { spec: string; options: any }[] = [];
  const context = {
    effect(factory: () => () => void) { disposers.push(factory()); }, on() { return () => {}; },
    locale: { register() { return () => {}; }, bind() { return (key: string) => key; } },
    ...
    remote: { $on() { return () => {}; }, pluginInventory: { async list() { return { ok: true, value: { managementAvailable: true } }; } }, pluginManager: {
      async inspect() { return { ok: true, value: options.inspect?.() ?? { status: 'accepted', kind: 'registry', name: 'dsh-review-test', version: '1.2.3', bundle: true } }; },
  ...
  return { main, market, reviews, prepares, installs, snapshot: () => main.hooks.pluginManager.getSnapshot(), advance: (ms: number) => { now += ms; },
    begin(face = market, spec = 'dsh-review-test@latest') { face.openInstall(); face.editInstallSpec(spec); face.runInstall(); },
    dispose() { for (const dispose of disposers.reverse()) dispose(); },
  };
}

test('both install surfaces stop for a DSH report and require explicit acknowledgement before an exact-version install', async () => {
