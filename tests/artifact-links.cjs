...
async function run(event) {
  ...
  await sleep(250);
  assert.equal(svgVisible(), true, 'Computer controls above the SVG hid the preview');
  await js(`document.querySelector('.desktop-computer summary').click()`);
  report.checks.push('SVG stays visible when the computer popover opens above its viewport');
  ...
  await js(`document.querySelector('.desktop-computer summary').click()`);
  await sleep(250);
  assert.equal(window.contentView.children.find(view => view.webContents === pngPage)?.getVisible(), true, 'Computer controls above the PNG hid the preview');
  assert.equal(await pngPage.executeJavaScript(`document.querySelector('img').complete && document.querySelector('img').naturalWidth === 32`), true);
  await js(`document.querySelector('.desktop-computer summary').click()`);
