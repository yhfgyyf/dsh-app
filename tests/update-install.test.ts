...
import { holdWindowsDll } from './windows-dll-fixture.ts';
...
test('Windows installer failure restores and relaunches the old app while its DLL is still mapped', windows, async () => {
  ...
  const holder = await holdWindowsDll(dll, directory);
  try {
    ...
    assert.equal(holder.exitCode, null, 'The DLL must stay mapped throughout rollback');
