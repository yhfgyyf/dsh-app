...
export async function applyProgressiveImages({ runtimeNodeModules = join(root, '.runtime/node_modules'), backupHome = join(root, '.build-runtime'), mode = 'apply' } = {}) {
  ...
  if (sha(before) === pin.after) return mode === 'check' ? { pending: 0 } : { changed: false };
  if (sha(before) !== pin.before) throw new Error('Unrecognized progressive-tools changes preserved: ' + path);
  if (mode === 'check') return { pending: 1 };
  if (mode === 'verify') throw new Error('Progressive image forwarding patch is missing.');
  ...
  const backup = join(backupHome, 'backups', `progressive-images-${Date.now()}`);
