...
从 v0.1.2 起，App 支持“每次打开 App 检查一次”和“每日定时检查一次”两种模式，默认启动检查。每日模式可设置本地时间，在 App 运行时执行；检查范围包含本仓库已发布的 GitHub 预览版。也可在应用菜单“检查更新…”或“设置 → 通用 → 桌面应用”手动检查。发现新版后选择“下载更新”，校验完成后点击“重启并安装”。请先等待正在运行的任务完成；安装会保留会话、配置和旧版 App 备份。v0.1.1 及更早版本需要手动安装一次新版。
