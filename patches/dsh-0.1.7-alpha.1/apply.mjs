...
const pkg=join(globalRoot,'@deepseek-ai/dsh'),nm=process.env.DSH_PATCH_NODE_MODULES_ROOT??join(pkg,'node_modules');
...
const backup=join(process.env.DSH_HOME??join(homedir(),'.dsh'),'backups','local-fixes-0.1.7-alpha.1-'+new Date().toISOString().replace(/[:.]/g,'-'));await mkdir(backup,{recursive:true,mode:0o700});
