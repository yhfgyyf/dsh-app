...
export function withDesktopPlugin(value: unknown, revision: string): BootGraph {
  ...
  const result: BootGraph = {
    ...value,
    entries: [...value.entries, { id: DESKTOP_PLUGIN_ID, url, rev: revision, inject: required, external: ['react', '@deepseek-ai/dsh-client-ui-primitives', '@deepseek-ai/dsh-client-store'] }],
